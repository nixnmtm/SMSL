...
from __future__ import (absolute_import, division, print_function,
                        unicode_literals)
from future.builtins import (ascii, bytes, chr, filter, hex, input,
...
def readAlg(filename):
    ...
    sequences = list()
    notfirst = 0
    with open(filename, "r") as algnfile:
        for line in algnfile.readlines():
...
def AnnotPfam(pfam_in, pfam_out, pfam_seq=path2pfamseq):
    ...
    for i, key in enumerate(pfamseq_ids):
        print("Current step %i, key {}" % (i,key))
        try:
            info = seq_info[key]
        except:
            info = "\t".join(["unknown"]*10 + ["unknown;unknown"])
        #this print line works with older pfamseq.txt files (release 4 and before, was
        #used to annotate the tutorial alignments
        #print(">%s|%s|%s|%s\n" % (key, info.split("\t")[6], info.split("\t")[9],\
        #        ",".join([name.strip() for name in info.split("\t")[10].split(";")])))
        #this print line works with the new version of pfamseq.txt
...
def filterSeq(alg0, sref=0.5, max_fracgaps=.2, min_seqid=.2, max_seqid=.8):
    ...
    **Example:**
    ...
    print("Keeping {:d} sequences of {:d} sequences (after filtering for gaps)"% (len(seqkeep0), Nseq))
    ...
    print("Keeping {:d} sequences of {:d} sequences (after filtering for seq similarity)"% (len(seqkeep), len(seqkeep0)))
...
def figUnits(v1, v2, units, marker="o", dotsize=9, notinunits=1, ax=None):
    ...
    **Keyword Arguments**
       ...
       -  `ax` = axis for figure plotting; default is the global plot area.

    :Example:
      >>> figUnits(v1, v2, units, marker="o", gradcol=0, dotsize=9, notinunits=1)
     """
    ntot = len(v1)
    axes = plt if ax is None else ax
    ...
    if notinunits == 1:
        ...
        axes.plot(notinunits[0], notinunits[1], marker,
            markersize=dotsize,
            markerfacecolor="w",
            markeredgecolor="k"
        )
    ...
    for u in units:
        ...
        if u.col < 0:
            bgr = [.7, .7, .7]
        axes.plot(v1[np.ix_(items_list)], v2[np.ix_(items_list)], marker,
            markersize=dotsize,
            markerfacecolor=bgr,
            markeredgecolor="k"
        )
...
def writePymol(pdb, sectors, ics, ats, outfilename,
               chain="A", inpath=path2structures, quitline=1):
    ...
    with open(outfilename, "wb") as f:
        f.write("delete all\n".encode())
        f.write("load {}.pdb, main\n".format(path.join(inpath, pdb)).encode())
        f.write("hide all\n".encode())
        f.write("bg_color white\n".encode())
        f.write("show cartoon, (chain {})\n".format(chain).encode())
        f.write("color white\n\n".encode())
        for k, sec in enumerate(sectors):
            b, g, r = colorsys.hsv_to_rgb(sec.col, 1, 1)
            f.write(
                "set_color color{:d}, [{:.3f},{:.3f},{:.3f}]\n".format(
                    k+1, b, g, r).encode())
            f.write("create sector%i, (resi {}) & (chain {})\n".format(
                k+1, ",".join([ats[s] for s in sec.items]), chain).encode())
            f.write("color color{:d}, sector{:d}\n".format(k+1, k+1).encode())
            f.write("show spheres, sector{:d}\n".format(k+1).encode())
            f.write("show surface, sector{:d}\n\n".format(k+1).encode())
        for k, sec in enumerate(ics):
            b, g, r = colorsys.hsv_to_rgb(sec.col, 1, 1)
            f.write(
                "set_color color_ic{:d}, [{:.3f},{:.3f},{:.3f}]\n".format(
                    k+1, b, g, r).encode())
            f.write("create ic_{:d}, (resi {}) & (chain {})\n".format(
                k+1, ",".join([ats[s] for s in sec.items]), chain).encode())
            f.write("color color_ic{:d}, ic_{:d}\n".format(k+1, k+1).encode())
            f.write("show spheres, ic_{:d}\n".format(k+1).encode())
            f.write("show surface, ic_{:d}\n\n".format(k+1).encode())
        f.write("zoom".encode())
        f.write("set transparency, 0.4\n".encode())
        f.write("ray".encode())
        path_list = outfilename.split(os.sep)
        fn = path_list[-1]
        f.write("png {}\n".format(fn.replace(".pml","")).encode())
        if quitline == 1:
            f.write("quit\n".encode())
...
def cytoscapeOut(ats, cutoff, Csca, Di, sectors, Vp, outfilename):
    ...
    print("KEY\tCONSERVATION\tSector\tVp1\tVp2\tVp3\n")
    for j,k in enumerate(ats):
        print((k + "\t %.4f \t {:d} \t %.4f \t %.4f \t %.4f \n") %(Di[j], s_idx[j], Vp[j,0], Vp[j,1], Vp[j,2]))
    f.close()
